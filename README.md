...
리팩토링이 용이하고 일관성 있는 구조가 best-practice라는 결론이 나왔다. pages와 components 폴더를
주축으로 하고 api, hook 등 추가적으로 필요한 폴더를 추가하는 구조가 현재 우리가 해야할 코드 리팩토링에도 용이하고,
후에 확장성에도 제약이 적을 것이다.
style관련 파일을 따로 분류하는 것에 대한 논의가 있었는데, styled-components를 사용하기로 정하였고,
따로 파일로 분리하기 보다는 각 컴포넌트 안에서 관리하는 것이 직관적이고, 유지보수가 용이하다고 판단하였다.
추가적으로 컴포넌트 재사용성 고려 & 명확한 프로젝트 구조 유지를 위해 아토믹 디자인 패턴을 도입하자는 논의가 있었는데,
이미 기존의 코드가 있는 상태에서 아토믹 디자인 패턴에 따른 구조를 도입하기에는 시간상 무리가 있다고 결론을 내렸다.
```
...
```
axios 인스턴스를 생성하여 api를 따로 분리시킨 코드를 best-practice로 선정하였다.
api instance를 사용하여 코드의 재사용성을 높이고 api 로직을 따로 관리하는 것이 좋다는 것에 모두 동의하였다.
더불어, 불필요한 코드의 중복과 낭비를 막기 위해 axios interceptor를 사용하였다.
